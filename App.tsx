...
/**
 * ResponsiveTimeline Component
 * 
 * The main landing page component featuring an interactive timeline.
 * 
 * Features:
 * - Interactive timeline strip with drag-to-scroll functionality.
 * - Detail view for selected timeline eras (Master's, University, etc.).
 * - "Grid" vs "Detail" view states.
  ...
  return (
    <div className="relative h-full w-full bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-white overflow-hidden font-sans transition-colors duration-500">
      ...
      <div
        ...
        className={`absolute inset-0 z-10 flex flex-col transition-transform duration-700 ease-[cubic-bezier(0.25,1,0.5,1)] ${viewState === 'grid' ? 'translate-y-0 opacity-100' : '-translate-y-1/2 opacity-50 pointer-events-none'
          }`}
      >
        {/* 1. Background Layer */}
        <ImageMarquee />
      ...
      <div
        ...
        className={`absolute inset-0 z-30 bg-slate-50 dark:bg-slate-950 flex flex-col transition-transform duration-700 ease-[cubic-bezier(0.25,1,0.5,1)] ${viewState === 'detail' ? 'translate-y-0' : 'translate-y-[105%]'
          }`}
      >
 ...
 * 
 * A versatile layout component used for both Academic and Professional sections.
 * 
 * Features:
 * - Supports two variants: 'tabs' (horizontal navigation) and 'stack' (vertical list).
 * - Displays detailed modules/projects with rich text rendering (Markdown support).
 * - Includes a vertical image marquee (gallery) on the right side.
 * - Responsive design: Collapses to single column on smaller screens.
 * - Handles "Full Width" modules (like Thesis) by hiding the marquee.
 */
 ...
 * Defines the main sidebar navigation items and their sub-menus.
 */
 ...
 * 
 * Responsibilities:
 * - Manages global state: active tab, sub-tab, dark mode, language.
 * - Handles routing/rendering of different sections (Home, Academic, Profession, Skills).
 * - Implements the main layout shell with Sidebar and Content Area.
        ...
        <div className="flex-1 flex flex-col justify-center space-y-2 px-3">
          {NAV_SECTIONS.map((section) => (
            <button
              ...
              className={`flex items-center gap-4 p-3 rounded-xl transition-all duration-200 group ${activeTab === section.id
            ...
            <div className="space-y-2">
              {activeNavSection?.subItems?.map(sub => (
                <button
                  ...
                  className={`w-full text-left px-4 py-3 rounded-lg flex items-center justify-between transition-colors ${activeSubTab === sub.id
